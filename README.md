...
| CoT SFT | `分析：...\n答案：A` | 学习显式医学题推理过程 |
| CoT + GRPO | `分析 + 答案` | 用多目标 reward 强化正确率、答案格式和安全表达 |
...
当前 CoT/Mixed 在 CMExam-test 上相比 Base 仍有提升，但低于 Optimized Direct SFT。进一步诊断显示，这不是由答案抽取失败或输出截断导致；在 CMExam-test-multi 上，CoT/Mixed 抽取成功率为 1.0000，非法答案率为 0.0000。在当前非 paired 对照中，CoT/Mixed 的 option recall 从 Direct 的 0.8926 升至 0.9187，但 option precision 从 0.9318 降至 0.9043，extra option rate 从 0.1765 上升到 0.2549。需要注意：`cmb_optimized_direct` 与 `cmb_cot_mixed` 题目集合重叠率很低，旧结果不能严格归因于 CoT 输出格式，已构造 paired control datasets 供下一轮公平对照。
